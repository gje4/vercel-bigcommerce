...
import { ShopifyDataGenerator } from '@/components/shopify-data-generator'
...
export function HomePageContent({
  ...
  // Show error toast if GitHub OAuth is not configured
  useEffect(() => {
    if (searchParams.get('error') === 'github_not_configured') {
      toast.error('GitHub OAuth not configured', {
        description: 'GitHub sign-in is not available. Please contact the administrator or use Vercel sign-in instead.',
      })
      // Remove the query parameter from URL
      const newUrl = new URL(window.location.href)
  ...
  // Show error toast if Vercel authentication failed
  useEffect(() => {
    if (searchParams.get('error') === 'vercel_auth_failed') {
      toast.error('Vercel authentication failed', {
        description:
          'Unable to fetch user information from Vercel. Please try signing in again or contact support if the issue persists.',
      })
      // Remove the query parameter from URL
      const newUrl = new URL(window.location.href)
      newUrl.searchParams.delete('error')
  ...
  // Show error toast if database setup is required
  useEffect(() => {
    if (searchParams.get('error') === 'db_setup_required') {
      toast.error('Database setup required', {
        description: 'Database tables are missing. Please run migrations with: pnpm db:migrate',
    ...
    if (isMultiAgent) {
        ...
        const responses = await Promise.all(
          tasksData.map((taskData) =>
            fetch('/api/tasks', {
              ...
              body: JSON.stringify({ ...taskData, images: data.images }),
            }),
          ),
        )
      ...
      try {
        const response = await fetch('/api/tasks', {
          ...
          body: JSON.stringify({ ...data, id, images: data.images }), // Include the pre-generated ID and images
        })
  ...
  return (
    <div className="flex-1 bg-background flex flex-col">
      {/* Two-column layout: Shopify Generator (50%) | Existing Content (50%) */}
      <div className="flex-1 flex flex-col lg:flex-row">
        {/* Left side: Store Generator (50%) */}
        <div className="flex-1 lg:w-1/2 overflow-y-auto">
          <ShopifyDataGenerator />
        </div>

        {/* Right side: Existing Content (50%) */}
