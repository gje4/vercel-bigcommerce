export function validateEnvironmentVariables(
  ...
  // V0 workflow doesn't require agent API keys - it uses V0 Platform API directly
  if (selectedAgent === 'v0') {
    // No API key validation needed for V0 workflow
  } else if (selectedAgent === 'claude' && !apiKeys?.ANTHROPIC_API_KEY && !process.env.ANTHROPIC_API_KEY) {
    errors.push('ANTHROPIC_API_KEY is required for Claude CLI. Please add your API key in your profile.')
  } else if (selectedAgent === 'cursor' && !apiKeys?.CURSOR_API_KEY && !process.env.CURSOR_API_KEY) {
    errors.push('CURSOR_API_KEY is required for Cursor CLI. Please add your API key in your profile.')
  } else if (selectedAgent === 'codex' && !apiKeys?.AI_GATEWAY_API_KEY && !process.env.AI_GATEWAY_API_KEY) {
    errors.push('AI_GATEWAY_API_KEY is required for Codex CLI. Please add your API key in your profile.')
  } else if (selectedAgent === 'gemini' && !apiKeys?.GEMINI_API_KEY && !process.env.GEMINI_API_KEY) {
