...
export async function pushChangesToBranch(
  ...
  try {
    ...
    if (!statusResult.output?.trim()) {
      ...
      // For new repos, we still need to push even if there are no changes (initial commit)
      if (isNewRepo) {
        await logger.info('New repository: pushing initial state to main branch')
      } else {
        return { success: true }
      }
    } else {
    ...
    // Push to remote branch (main for new repos, specified branch for existing repos)
    const targetBranch = isNewRepo ? 'main' : branchName
    const pushArgs = isNewRepo ? ['push', '-u', 'origin', targetBranch] : ['push', 'origin', targetBranch]
    const pushResult = await runInProject(sandbox, 'git', pushArgs)

    if (pushResult.success) {
      await logger.info(`Successfully pushed changes to ${targetBranch}`)
      return { success: true }
    } else {
      const errorMsg = pushResult.error || 'Unknown error'
      await logger.info(`Failed to push to ${targetBranch}`)
