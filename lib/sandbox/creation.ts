...
export async function createSandbox(config: SandboxConfig, logger: TaskLogger): Promise<SandboxResult> {
  try {
    ...
    try {
      ...
      // Handle new repo creation vs cloning existing repo
      if (config.createNewRepo) {
        // For new repos, initialize fresh git repo instead of cloning
        await logger.info('Initializing new repository (no clone needed)...')
        ...
        // Initialize git repo
        const gitInitResult = await runCommandInSandbox(sandbox, 'git', ['init'], PROJECT_DIR)
        if (!gitInitResult.success) {
          throw new Error('Failed to initialize Git repository')
        }

        // Set up remote to point to new repo
        const addRemoteResult = await runCommandInSandbox(sandbox, 'git', ['remote', 'add', 'origin', authenticatedRepoUrl], PROJECT_DIR)
        if (!addRemoteResult.success) {
          // If remote already exists, set it
          const setRemoteResult = await runCommandInSandbox(sandbox, 'git', ['remote', 'set-url', 'origin', authenticatedRepoUrl], PROJECT_DIR)
          if (!setRemoteResult.success) {
            throw new Error('Failed to set Git remote')
          }
        }

        await logger.info('New repository initialized successfully')

        // Call progress callback after initialization
        if (config.onProgress) {
          await config.onProgress(30, 'New repository initialized, ready for code generation...')
        }
      } else {
...
`,
    ...
    // For new repos, use main branch (no branch creation needed)
    if (config.createNewRepo) {
      // Ensure we're on main branch
      const checkoutMain = await runInProject(sandbox, 'git', ['checkout', '-b', 'main'])
      if (!checkoutMain.success) {
        // Try to checkout existing main if it exists
        const checkoutExistingMain = await runInProject(sandbox, 'git', ['checkout', 'main'])
        if (!checkoutExistingMain.success) {
          throw new Error('Failed to checkout main branch')
        }
      }
      branchName = 'main'
      await logger.info('Using main branch for new repository')
