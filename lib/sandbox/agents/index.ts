...
import { executeV0InSandbox } from './v0'
import { TaskLogger } from '@/lib/utils/task-logger'
import { Connector } from '@/lib/db/schema'

export type AgentType = 'claude' | 'codex' | 'copilot' | 'cursor' | 'gemini' | 'opencode' | 'v0'
...
export async function executeAgentInSandbox(
  ...
  if (apiKeys?.V0_API_KEY) process.env.V0_API_KEY = apiKeys.V0_API_KEY
  if (githubToken) {
  ...
  try {
    switch (agentType) {
      ...
      case 'gemini':
        return await executeGeminiInSandbox(sandbox, instruction, logger, selectedModel, mcpServers, images)
      ...
      case 'v0':
        return await executeV0InSandbox(
          sandbox,
          instruction,
          logger,
          selectedModel,
    ...
    process.env.V0_API_KEY = originalEnv.V0_API_KEY
