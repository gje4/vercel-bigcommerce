...
export async function executeClaudeInSandbox(
  ...
  try {
    ...
    if (images && images.length > 0) {
      if (logger) {
        await logger.info('Processing images for Claude CLI')
      }
      const tempDir = '/tmp/task-images'
      await runCommandInSandbox(sandbox, 'mkdir', ['-p', tempDir])
            ...
            : mimeType.includes('png')
              ...
              : mimeType.includes('gif')
                ...
                : mimeType.includes('webp')
                  ? 'webp'
                  : 'png'
        const imagePath = `${tempDir}/image-${i + 1}.${extension}`

        // Write base64 data to file using base64 decode (safe - base64 only contains A-Z, a-z, 0-9, +, /, =)
        // Escape single quotes in the path for safety
        const escapedPath = imagePath.replace(/'/g, "'\\''")
        const writeCmd = `echo '${base64Data}' | base64 -d > '${escapedPath}'`
        const writeResult = await runCommandInSandbox(sandbox, 'sh', ['-c', writeCmd])
        if (writeResult.success) {
          imagePaths.push(imagePath)
          if (logger) {
            await logger.info(`Image ${i + 1} saved to sandbox`)
          }
        } else {
          if (logger) {
            await logger.error(`Failed to save image ${i + 1}`)
          }
        }
      }

      if (imagePaths.length > 0 && logger) {
        await logger.info(`Successfully processed ${imagePaths.length} image(s)`)
      }
    }
    ...
    // Add image file paths if available (Claude CLI supports --image flag)
    if (imagePaths.length > 0) {
      for (const imagePath of imagePaths) {
        fullCommand += ` --image "${imagePath}"`
      }
      if (logger) {
        await logger.info(`Including ${imagePaths.length} image(s) in prompt`)
      }
    }
