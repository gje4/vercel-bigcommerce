...
export const tasks = pgTable('tasks', {
  ...
  images: jsonb('images').$type<string[] | null>(), // Array of base64-encoded image data URIs (optional)
  v0ProjectUrl: text('v0_project_url'), // v0 Platform API project URL (optional)
  createNewRepo: boolean('create_new_repo').default(false), // Whether to create new repo instead of branch
  newRepoOwner: text('new_repo_owner'), // Owner/organization for new repo (optional, defaults to user)
  originalRepoUrl: text('original_repo_url'), // Store original repo URL when creating new repo
  createdAt: timestamp('created_at').defaultNow().notNull(),
  updatedAt: timestamp('updated_at').defaultNow().notNull(),
...
export const insertTaskSchema = z.object({
  ...
  selectedAgent: z.enum(['claude', 'codex', 'copilot', 'cursor', 'gemini', 'opencode', 'v0']).default('claude'),
  ...
  images: z.array(z.string()).optional(), // Array of base64-encoded image data URIs
  v0ProjectUrl: z.string().url().optional(), // v0 Platform API project URL
  createNewRepo: z.boolean().default(false).optional(), // Whether to create new repo instead of branch
  newRepoOwner: z.string().optional(), // Owner/organization for new repo
  originalRepoUrl: z.string().url().optional(), // Store original repo URL when creating new repo
  createdAt: z.date().optional(),
  updatedAt: z.date().optional(),
...
export const selectTaskSchema = z.object({
  ...
  images: z.array(z.string()).nullable(), // Array of base64-encoded image data URIs
  v0ProjectUrl: z.string().nullable(), // v0 Platform API project URL
  createNewRepo: z.boolean().nullable(), // Whether to create new repo instead of branch
  newRepoOwner: z.string().nullable(), // Owner/organization for new repo
  originalRepoUrl: z.string().nullable(), // Store original repo URL when creating new repo
  createdAt: z.date(),
  updatedAt: z.date(),
...
export const keys = pgTable(
  'keys',
  {
    ...
    userId: text('user_id')
      ...
      .references(() => users.id, { onDelete: 'cascade' }), // Foreign key to users table
    provider: text('provider', {
      enum: ['anthropic', 'openai', 'cursor', 'gemini', 'aigateway', 'v0'],
    }).notNull(),
